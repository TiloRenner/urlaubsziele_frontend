export default function getCountry(countryCode) {
  ...
  const country = countryArray.filter((c) => c.id == countryCode)
  let countryTraverse = []
  let countryType = country[0].geometry.type
  if (countryType == "Polygon") {
    countryTraverse.push(country[0].geometry.coordinates[0].map((coord) => {
        return([coord[1], coord[0]])
       } ))
  } else {
    for (let part of country[0].geometry.coordinates) {
      countryTraverse.push(part[0].map((coord) => {
        return([coord[1], coord[0]])
       } ))
    }
  }

  return {geometry: countryTraverse, type: countryType}
}
